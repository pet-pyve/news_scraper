...
while len(links) < 2000: #only grab 2000 urls to search through
    ...
    news_page = "https://www.breitbart.com/news/source/breitbart-news/page/" + str(num) + "/"   #crafting the url
    ...
    articles = soup.findAll("div",  attrs={"class": "article-content"})
...
for tasty_link in links: #for every link in the list
    ...
    ws.cell(row=row, column=column).value = "breitbart" #from the abc
    column += 1
